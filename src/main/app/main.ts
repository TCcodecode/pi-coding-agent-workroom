...
function registerPiIpc() {
  ...
  ipcMain.handle(
    "pi:rewindLastTurn",
    (_event, opts?: { sessionKey?: string }) => piHost.rewindLastTurn(opts),
  );
  ...
  ipcMain.handle("pi:setThinkingLevel", (_event, level, opts?: { sessionKey?: string }) => piHost.setThinkingLevel(level, opts));
  ...
  ipcMain.handle("pi:setModel", (_event, model: string, opts?: { sessionKey?: string }) => piHost.setModel(model, opts));
...
app.whenReady().then(() => {
  ...
  companion = new CompanionGateway({
    userDataDir,
    invoke: createCompanionInvoker(piHost, { getGitBranch }),
