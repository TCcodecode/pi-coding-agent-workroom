...
import { deleteSessionFile, getSessionContext, listSessions as loadSessionCatalog } from "./catalog.js";
import {
  addProject,
  getActiveProjectId,
  listProjects,
...
export class PiHost {
  ...
  async undoFileChange(path: string, opts?: SessionCommandOptions): Promise<void> {
    ...
    this.restoreTextFile(mutation.absolutePath, mutation.before);
  ...
  setThinkingLevel(level: ThinkingLevel, opts?: SessionCommandOptions): void {
    const slot = this.requireSlot(opts?.sessionKey);
    slot.runtime.session.setThinkingLevel(level);
    this.emit("thinking_level_changed", { level }, undefined, slot.key);
  }
  ...
  setTools(tools: string[], opts?: SessionCommandOptions): void {
    ...
    slot.runtime.session.setActiveToolsByName(requested);
  }
  ...
  async setModel(modelKey: string, opts?: SessionCommandOptions): Promise<void> {
    const slot = this.requireSlot(opts?.sessionKey);
    const session = slot.runtime.session;
    ...
    this.emit("model_changed", { model: modelKey, provider }, undefined, slot.key);
  }
  ...
  ): Promise<{ editorText?: string; cancelled: boolean; snapshot: PiSnapshot }> {
    const slot = this.requireSlot(opts?.sessionKey);
    const session = slot.runtime.session;
    if (!session.navigateTree) {
      throw new Error("Pi runtime cannot navigate the session tree");
    }
    if (session.isStreaming) {
      await session.abort();
    }
    const result = await session.navigateTree(entryId);
    const previousKey = this.foregroundKey;
    this.foregroundKey = slot.key;
    try {
      return {
      ...
      this.foregroundKey = previousKey;
    }
  }

  async rewindLastTurn(
    opts?: SessionCommandOptions,
  ): Promise<{ editorText?: string; cancelled: boolean; snapshot: PiSnapshot }> {
    const slot = this.requireSlot(opts?.sessionKey);
    const entryId = slot.lastTurnUserEntryId;
    if (!entryId) throw new Error("Nothing to undo");
    const session = slot.runtime.session;
    if (!session.navigateTree) {
      throw new Error("Pi runtime cannot navigate the session tree");
    }
    ...
    this.restoreLastTurnFiles(slot);
    const result = await session.navigateTree(entryId);
    if (result.cancelled !== true) {
      slot.lastTurnUserEntryId = undefined;
      slot.lastTurnFileSnapshots.clear();
  ...
  snapshot(opts?: { includeTimeline?: boolean; tailTurns?: number }): PiSnapshot {
    return buildSnapshot({
      ...
      lastTurnEntryId: this.getSlot()?.lastTurnUserEntryId,
    });
  }
   ...
   * model often tracks tasks with todowrite but forgets to close its
  ...
  private restoreTextFile(absolutePath: string, before: string | undefined): void {
    if (before === undefined) {
      try {
  ...
  private restoreLastTurnFiles(slot: RuntimeSlot): void {
    const restores = new Map(slot.lastTurnFileSnapshots);
    for (const pending of slot.pendingFileMutations.values()) {
      if (!restores.has(pending.path)) restores.set(pending.path, pending);
    }
    for (const mutation of restores.values()) {
      this.restoreTextFile(mutation.absolutePath, mutation.before);
      const completed = slot.completedFileMutations.get(mutation.path);
      if (completed) {
        if (completed.before === mutation.before) {
          slot.completedFileMutations.delete(mutation.path);
        } else {
          completed.after = mutation.before;
        }
      }
      this.emit("file_change_undone", { path: mutation.path }, undefined, slot.key);
    }
  ...
  private async selectAvailableModel(providerId?: string): Promise<void> {
    ...
    if (this.modelName(session) === modelKey && session.thinkingLevel === thinkingLevel) {
      this.persistDefaultModelSelection(modelKey);
      return;
    }
    if (this.modelName(session) !== modelKey && session.setModel) {
      await session.setModel(selectedModel);
      this.emit("model_changed", { model: modelKey, provider: selected.provider }, undefined, slot.key);
    }
    if (session.thinkingLevel !== thinkingLevel) {
      session.setThinkingLevel(thinkingLevel);
      this.emit("thinking_level_changed", { level: thinkingLevel }, undefined, slot.key);
    }
    this.persistDefaultModelSelection(modelKey);
  }
