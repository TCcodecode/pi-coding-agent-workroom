...
export function finishFileMutation(
  ...
  if (change) {
    ...
    if (!slot.lastTurnFileSnapshots.has(mutation.path)) {
      slot.lastTurnFileSnapshots.set(mutation.path, {
        path: mutation.path,
        absolutePath: mutation.absolutePath,
  ...
  switch (event.type) {
    case "message_start": {
      ...
      if (event.message?.role === "assistant") {
        ...
        const entryId = slot.runtime.session.sessionManager?.getLeafId?.() ?? undefined;
        slot.lastTurnUserEntryId = entryId;
        slot.lastTurnFileSnapshots.clear();
        host.emit(
          "user_message_created",
          { messageId, content: messageText(event.message), ...(entryId ? { entryId } : {}) },
          raw,
          key,
        );
      }
