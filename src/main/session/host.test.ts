...
function createFakeRuntime(cwd = "/tmp/project", history: unknown[] = []) {
  ...
  const session: PiSessionLike & {
    ...
    abort: async () => { calls.push({ method: "abort", args: [] }); streaming = false; },
    ...
    navigateTree: async (entryId: string) => {
      calls.push({ method: "navigateTree", args: [entryId] });
      if (streaming) throw new Error("Wait for the current response to finish before navigating the session tree.");
      const userIndex = history.findIndex((item) => (item as { id?: string }).id === entryId);
      if (userIndex >= 0) history.splice(userIndex);
      return { editorText: "rewound prompt", cancelled: false };
    },
...
describe("PiHost", () => {
  ...
  test("rewindLastTurn restores this turn's files and navigates the last user message", async () => {
    const project = mkdtempSync(join(tmpdir(), "pi-rewind-last-"));
    mkdirSync(join(project, "src"));
    const file = join(project, "src", "App.tsx");
    writeFileSync(file, "before\n");
    try {
      const fake = createFakeRuntime(project, [
        { role: "user", id: "aaaaaaaa", content: "change the file" },
        { role: "assistant", id: "a1", content: "ok" },
      ]);
      fake.setLeafId("aaaaaaaa");
      const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });

      fake.emit({
        type: "message_start",
        message: { role: "user", id: "m1", content: "change the file" },
      });
      fake.emit({ type: "tool_execution_start", toolCallId: "edit-1", toolName: "edit", args: { path: "src/App.tsx" } });
      writeFileSync(file, "after\n");
      fake.emit({ type: "tool_execution_end", toolCallId: "edit-1", toolName: "edit", isError: false, result: "ok" });

      expect(host.snapshot().session.lastTurnEntryId).toBe("aaaaaaaa");

      const result = await host.rewindLastTurn();

      expect(readFileSync(file, "utf8")).toBe("before\n");
      expect(fake.calls.map((call) => call.method)).toEqual(["navigateTree"]);
      expect(fake.calls[0]?.args).toEqual(["aaaaaaaa"]);
      expect(result.cancelled).toBe(false);
      expect(result.editorText).toBe("rewound prompt");
      expect(result.snapshot.session.lastTurnEntryId).toBeUndefined();
    } finally {
      rmSync(project, { recursive: true, force: true });
    }
  });

  test("rewindLastTurn only restores the latest turn's files", async () => {
    const project = mkdtempSync(join(tmpdir(), "pi-rewind-turns-"));
    mkdirSync(join(project, "src"));
    const first = join(project, "src", "first.ts");
    const second = join(project, "src", "second.ts");
    writeFileSync(first, "orig-first\n");
    writeFileSync(second, "orig-second\n");
    try {
      const fake = createFakeRuntime(project);
      const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });

      fake.setLeafId("turn0001");
      fake.emit({ type: "message_start", message: { role: "user", id: "m1", content: "edit first" } });
      fake.emit({ type: "tool_execution_start", toolCallId: "e1", toolName: "edit", args: { path: "src/first.ts" } });
      writeFileSync(first, "turn-1\n");
      fake.emit({ type: "tool_execution_end", toolCallId: "e1", toolName: "edit", isError: false, result: "ok" });

      fake.setLeafId("turn0002");
      fake.emit({ type: "message_start", message: { role: "user", id: "m2", content: "edit second" } });
      fake.emit({ type: "tool_execution_start", toolCallId: "e2", toolName: "edit", args: { path: "src/second.ts" } });
      writeFileSync(second, "turn-2\n");
      fake.emit({ type: "tool_execution_end", toolCallId: "e2", toolName: "edit", isError: false, result: "ok" });

      await host.rewindLastTurn();

      expect(readFileSync(first, "utf8")).toBe("turn-1\n");
      expect(readFileSync(second, "utf8")).toBe("orig-second\n");
      expect(fake.calls.filter((call) => call.method === "navigateTree").map((call) => call.args)).toEqual([["turn0002"]]);
      await expect(host.rewindLastTurn()).rejects.toThrow("Nothing to undo");
    } finally {
      rmSync(project, { recursive: true, force: true });
    }
  });

  test("rewindLastTurn unlinks a file created this turn", async () => {
    const project = mkdtempSync(join(tmpdir(), "pi-rewind-create-"));
    const created = join(project, "new.ts");
    try {
      const fake = createFakeRuntime(project);
      fake.setLeafId("created1");
      const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });

      fake.emit({ type: "message_start", message: { role: "user", id: "m1", content: "create file" } });
      fake.emit({ type: "tool_execution_start", toolCallId: "w1", toolName: "write", args: { path: "new.ts" } });
      writeFileSync(created, "brand new\n");
      fake.emit({ type: "tool_execution_end", toolCallId: "w1", toolName: "write", isError: false, result: "ok" });
  ...
  test("rewindLastTurn restores a pending mutation when aborting a running turn", async () => {
    const project = mkdtempSync(join(tmpdir(), "pi-rewind-pending-"));
    const file = join(project, "src.ts");
    writeFileSync(file, "before\n");
    try {
      const fake = createFakeRuntime(project);
      fake.setLeafId("pending1");
      fake.setStreaming(true);
      const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });

      fake.emit({ type: "message_start", message: { role: "user", id: "m1", content: "edit now" } });
      fake.emit({ type: "tool_execution_start", toolCallId: "e1", toolName: "edit", args: { path: "src.ts" } });
      writeFileSync(file, "partial\n");

      await host.rewindLastTurn();

      expect(readFileSync(file, "utf8")).toBe("before\n");
      expect(fake.calls.map((call) => call.method)).toEqual(["abort", "navigateTree"]);
    } finally {
      rmSync(project, { recursive: true, force: true });
    }
  });

  test("rewindLastTurn undoes conversation when this turn did not touch files", async () => {
    const fake = createFakeRuntime();
    fake.setLeafId("onlychat");
    const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });
    fake.emit({ type: "message_start", message: { role: "user", id: "m1", content: "just ask" } });

    const result = await host.rewindLastTurn();

    expect(fake.calls.map((call) => call.method)).toEqual(["navigateTree"]);
    expect(fake.calls[0]?.args).toEqual(["onlychat"]);
    expect(result.editorText).toBe("rewound prompt");
  });

  test("rewindLastTurn throws when this process has no last turn", async () => {
    const fake = createFakeRuntime();
    const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });
    await expect(host.rewindLastTurn()).rejects.toThrow("Nothing to undo");
  });

  test("navigateTree aborts a streaming session then rewinds the snapshot", async () => {
    const history = [
      { role: "user", id: "e5f6a7b8", content: "first" },
      { role: "assistant", id: "a0", content: "ok" },
      { role: "user", id: "22222222", content: "second" },
      { role: "assistant", id: "a1", content: "later" },
    ];
    const fake = createFakeRuntime("/p", history);
    fake.setStreaming(true);
    const host = new PiHost({ workspaceId: "workspace-1", runtime: fake.runtime });

    const result = await host.navigateTree("e5f6a7b8");

    expect(fake.calls.map((call) => call.method)).toEqual(["abort", "navigateTree"]);
    expect(result.cancelled).toBe(false);
    expect(result.editorText).toBe("rewound prompt");
    expect(result.snapshot.timeline.some((item) => item.kind === "user" && item.content === "first")).toBe(false);
    expect(result.snapshot.timeline.some((item) => item.kind === "user" && item.content === "second")).toBe(false);
  });

  test("navigateTree with sessionKey targets that slot", async () => {
    const historyA = [{ role: "user", id: "aaaaaaa1", content: "keep-a" }];
    const historyB = [
      { role: "user", id: "bbbbbbb1", content: "first-b" },
      { role: "user", id: "bbbbbbb2", content: "second-b" },
    ];
    const host = new PiHost({
      workspaceId: "workspace-1",
      runtimeFactory: async (opts) => {
        const history = opts.sessionPath?.includes("b.jsonl") ? historyB : historyA;
        const fake = createFakeRuntime(opts.cwd ?? "/p", history);
        fake.session.sessionFile = opts.sessionPath;
        fake.session.sessionId = opts.sessionPath?.includes("b.jsonl") ? "sid-b" : "sid-a";
        return fake.runtime;
      },
    });
    await host.start({ cwd: "/p", sessionPath: "/tmp/a.jsonl", sessionKey: "file:/tmp/a.jsonl" });
    ...
    const result = await host.navigateTree("bbbbbbb1", { sessionKey: "file:/tmp/b.jsonl" });

    expect(result.snapshot.timeline.some((item) => item.kind === "user" && item.content === "first-b")).toBe(false);
    const foreground = host.snapshot();
    expect(foreground.session.sessionFile).toBe("/tmp/b.jsonl");
    await host.focusSession("file:/tmp/a.jsonl", { includeTimeline: true });
    expect(host.snapshot().timeline.some((item) => item.kind === "user" && item.content === "keep-a")).toBe(true);
  });
