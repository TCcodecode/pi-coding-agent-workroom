...
describe("hydrateTimeline tail", () => {
  ...
  test("stamps SessionManager entry ids onto user rows", () => {
    const session = sessionWithTurns(2);
    session.sessionManager = {
      getTree: () => [],
      buildContextEntries: () => [
        { id: "a1b2c3d4", type: "model_change" },
        { id: "e5f6a7b8", type: "message", message: { role: "user" } },
        { id: "11111111", type: "message", message: { role: "assistant" } },
        { id: "22222222", type: "message", message: { role: "user" } },
        { id: "33333333", type: "message", message: { role: "assistant" } },
      ],
    };
    const items = hydrateTimeline(session, { tailTurns: Number.POSITIVE_INFINITY });
    const users = items.filter((item) => item.kind === "user");
    expect(users).toEqual([
      expect.objectContaining({ id: "u0", entryId: "e5f6a7b8" }),
      expect.objectContaining({ id: "u1", entryId: "22222222" }),
    ]);
  });

  test("leaves entryId off when the session has no manager", () => {
    const items = hydrateTimeline(sessionWithTurns(1), { tailTurns: Number.POSITIVE_INFINITY });
    const user = items.find((item) => item.kind === "user") as { id: string; entryId?: string } | undefined;
    expect(user?.id).toBe("u0");
    expect(user?.entryId).toBeUndefined();
  });

  test("maps tailed user rows to the matching slice of entry ids", () => {
    const session = sessionWithTurns(40);
    const userEntries = Array.from({ length: 40 }, (_, index) => ({
    ...
    session.sessionManager = {
      getTree: () => [],
      buildContextEntries: () => userEntries.flatMap((entry, index) => [
        entry,
        { id: `asst${index}`, type: "message", message: { role: "assistant" } },
      ]),
    };
    const items = hydrateTimeline(session);
    const users = items.filter((item) => item.kind === "user");
    expect(users).toHaveLength(30);
    expect(users[0]).toEqual(expect.objectContaining({ id: "u10", entryId: "user0010" }));
    expect(users[29]).toEqual(expect.objectContaining({ id: "u39", entryId: "user0039" }));
  });
