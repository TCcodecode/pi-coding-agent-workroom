...
function userEntryIds(session?: PiSessionLike): string[] {
  const entries = session?.sessionManager?.buildContextEntries?.() ?? [];
  const ids: string[] = [];
  for (const entry of entries) {
    if (entry.type === "message" && entry.message?.role === "user") ids.push(entry.id);
  }
  return ids;
}
...
function appendHydratedMessage(
  ...
  entryIds: { ids: string[]; next: number },
): void {
  const baseId = baseIdFor(message, index);
  ...
  if (role === "user") {
    ...
    const entryId = isCountableUser(message) ? entryIds.ids[entryIds.next++] : undefined;
    if (content.trim()) {
      items.push({
        id: baseId,
        kind: "user",
...
function hydrateRange(
  ...
  for (let index = 0; index < start; index += 1) {
    if (isCountableUser(messages[index]!)) usersBefore += 1;
  }
  const entryIds = { ids: userEntryIds(session), next: usersBefore };
  for (let index = start; index < end; index += 1) {
    appendHydratedMessage(messages[index]!, index, items, toolPaths, entryIds);
  }
  return items;
}
...
export function hydrateAllTurns(session?: PiSessionLike): TimelineItem[] {
  ...
  return hydrateRange(messages, 0, messages.length, session);
}
...
export function hydrateTimeline(
  ...
  return hydrateRange(messages, start, messages.length, session);
}
...
export function loadOlderItems(
  ...
  return { items: hydrateRange(messages, start, beforeIndex, session), hasMore: hasUserBefore(messages, start) };
}
