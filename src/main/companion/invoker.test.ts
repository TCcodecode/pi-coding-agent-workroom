...
describe("companion invoker", () => {
  ...
  test("returns the current git branch through the companion method", async () => {
    const getGitBranch = vi.fn(async () => "feature/mobile");
    const invoke = createCompanionInvoker({
      snapshot: () => ({}),
      listSessions: async () => [],
      prompt: async () => undefined,
    ...
    await expect(invoke("getGitBranch", ["/tmp/project"])).resolves.toBe("feature/mobile");
    expect(getGitBranch).toHaveBeenCalledWith("/tmp/project");
  });
