...
describe("sessionTabs", () => {
  ...
  test("togglePin moves tab to front when pinned", () => {
    let state = upsertTab([], {
      sessionId: "a",
      sessionFile: "/tmp/a.jsonl",
      projectId: "/p",
    ...
    const bId = state.tabs.find((t) => t.title === "B")!.id;
    const pinned = togglePinTab(state.tabs, bId);
    expect(pinned[0]?.title).toBe("B");
    expect(pinned[0]?.pinned).toBe(true);
    expect(pinned[1]?.title).toBe("A");
  });
