...
function makeFakeApi() {
  ...
  const api: PiApi = {
    ...
    rewindLastTurn: vi.fn(async () => ({
      cancelled: false,
      editorText: "retry this",
      snapshot: {
        workspaceId: "local",
        session: { ...useAppStore.getState().session, lastTurnEntryId: undefined },
        sessions: useAppStore.getState().sessions,
        projects: useAppStore.getState().projects ?? [],
        activeProjectId: useAppStore.getState().activeProjectId,
        timeline: useAppStore.getState().timeline.filter((item) => item.id !== "user-2" && item.id !== "assistant-2"),
        toolCalls: useAppStore.getState().toolCalls,
        queue: useAppStore.getState().queue,
        resources: { contextFiles: [], skills: [], promptTemplates: [], themes: [], extensions: [], packages: [] },
        diagnostics: { piVersion: "test", sequence: 0, messages: [], errors: [] },
...
describe("Pi Workroom end-to-end send flow", () => {
  ...
  test("undoes only the last turn into the composer without sending", async () => {
    const { api } = makeFakeApi();
    (window as unknown as { pi: PiApi }).pi = api;
    useWorkspaceStore.setState({
      ...
      activeTabId: "file:/tmp/s.jsonl",
      liveSessions: [{
        sessionKey: "file:/tmp/s.jsonl",
        sessionId: "s1",
        sessionFile: "/tmp/s.jsonl",
        cwd: "/tmp/project",
        projectId: "/tmp/project",
    ...
    useAppStore.setState({
      ...
      session: {
        ...
        cwd: "/tmp/project",
        status: "idle",
        lastTurnEntryId: "bbbbbbbb",
      },
      projects: [{ id: "/tmp/project", name: "project", path: "/tmp/project", updatedAt: new Date().toISOString() }],
      activeProjectId: "/tmp/project",
      timeline: [
        { id: "user-1", kind: "user", content: "first prompt", status: "completed", entryId: "aaaaaaaa" },
        { id: "assistant-1", kind: "assistant", content: "done", status: "completed" },
        { id: "user-2", kind: "user", content: "retry this", status: "completed", entryId: "bbbbbbbb" },
      ],
    });

    render(<App />);
    expect(screen.getAllByRole("button", { name: "Undo last turn" })).toHaveLength(1);
    fireEvent.click(screen.getByRole("button", { name: "Undo last turn" }));

    await waitFor(() => expect(api.rewindLastTurn).toHaveBeenCalledWith({ sessionKey: "file:/tmp/s.jsonl" }));
    expect(api.prompt).not.toHaveBeenCalled();
    expect(screen.getByRole("textbox", { name: /message/i })).toHaveValue("retry this");

    delete (window as unknown as { pi?: PiApi }).pi;
  });

  test("still exposes copy for the latest user message after a stopped partial assistant reply", async () => {
    const { api } = makeFakeApi();
    (window as unknown as { pi: PiApi }).pi = api;
    useAppStore.setState({
      ...createInitialState(),
    ...
    expect(screen.queryByRole("button", { name: /continue from here/i })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Undo last turn" })).not.toBeInTheDocument();
