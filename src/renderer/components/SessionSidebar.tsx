...
export interface SessionSidebarProps {
  ...
  onAddProject: () => void;
  /** Open the in-composer project picker for a new session (does not create yet). */
  onRequestNewSession: () => void;
  /** Create a new session immediately under this project (row + / context menu). */
  onNewSession: (projectId: string) => void;
  /** Mark project as active in the list (no session load). */
  onSelectProject: (projectId: string) => void;
  onSelectSession: (sessionPath: string, projectId: string, sessionId?: string) => void;
  onRenameSession: (sessionPath: string, name: string) => Promise<string>;
  onDeleteSession: (sessionPath: string, projectId: string) => Promise<void>;
  onCloneSession: (session: SessionSummary, projectId: string) => void;
...
function IconButton({
  ...
  const className = ["sidebar-icon-btn", accent ? "accent" : ""].filter(Boolean).join(" ");
  return (
...
export function SessionSidebar({
  ...
  const commitRename = async (session: SessionSummary, projectId: string) => {
    ...
    try {
      const resolved = await onRenameSession(session.sessionFile, next);
  ...
  const confirmDelete = async () => {
    ...
    setPendingDelete(null);
    try {
      await onDeleteSession(sessionFile, projectId);
      setSessionsByProject((prev) => ({
        ...prev,
                            ...
                            <ContextMenu.Root key={session.sessionId}>
                                ...
                                >
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      if (!path) return;
                                      onSelectSession(path, project.id, session.sessionId);
                                    }}
                              ...
                              <ContextMenu.Portal>
                                <ContextMenu.Content className="session-context-menu" alignOffset={4}>
                                  ...
                                  <ContextMenu.Item
                                    className="session-context-item"
                                    onSelect={() => onCloneSession(session, project.id)}
                                  >
                                    Duplicate session
                                  </ContextMenu.Item>
                                  {session.sessionFile ? (
