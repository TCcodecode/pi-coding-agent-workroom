...
function readHttpChatWidth(): number {
  try {
    const storedValue = localStorage.getItem(HTTP_CHAT_WIDTH_KEY);
    if (storedValue === null) return HTTP_CHAT_DEFAULT_WIDTH;
    ...
    return HTTP_CHAT_DEFAULT_WIDTH;
  }
}

function readHttpChatOpen(): boolean {
  try {
    return localStorage.getItem(HTTP_CHAT_OPEN_KEY) !== "false";
  } catch {
    return true;
  }
}
...
export function HttpWorkbench({
  ...
  const [chatWidth, setChatWidth] = useState(readHttpChatWidth);
  const [chatOpen, setChatOpen] = useState(readHttpChatOpen);

  useEffect(() => {
    try {
      localStorage.setItem(HTTP_CHAT_WIDTH_KEY, String(chatWidth));
    } catch {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(HTTP_CHAT_OPEN_KEY, String(chatOpen));
    } catch {
      // Ignore storage failures in restricted/test environments.
    }
  }, [chatOpen]);
  ...
  const resizeChatPanel = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    const onMove = (moveEvent: MouseEvent) => {
      setClampedChatWidth(startWidth - (moveEvent.clientX - startX));
    };
    const onUp = () => {
