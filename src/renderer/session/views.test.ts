...
describe("SessionView", () => {
  ...
  test("applySnapshotToView clears lastTurnEntryId when the snapshot omits it", () => {
    const view = createView("k", { hydrate: "ready", session: { lastTurnEntryId: "bbbbbbbb" } });
    const snap = {
      ...createInitialState(),
      session: { ...createInitialState().session, sessionId: "s1" },
    };
    const next = applySnapshotToView(view, snap);
    expect(next.session.lastTurnEntryId).toBeUndefined();
  });
