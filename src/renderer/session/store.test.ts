...
describe("reducePiEvent", () => {
  ...
  test("user_message_created records lastTurnEntryId for rewind", () => {
    let state = createInitialState();
    state = reducePiEvent(state, event("user_message_created", {
      messageId: "u1",
      content: "first",
    ...
    expect(state.session.lastTurnEntryId).toBe("aaaaaaaa");
    state = reducePiEvent(state, event("user_message_created", {
      messageId: "u2",
      content: "second",
      entryId: "bbbbbbbb",
    }));
    expect(state.session.lastTurnEntryId).toBe("bbbbbbbb");
  });
...
describe("applyEvent delta coalescing", () => {
  ...
  test("clips streaming thinking to 8KB", () => {
    let state = createInitialState();
    state = reducePiEvent(state, event("thinking_started", { messageId: "t1" }));
    state = reducePiEvent(state, event("thinking_delta", { messageId: "t1", delta: "a".repeat(9000) }));
    const item = state.timeline.find((row) => row.id === "t1");
    expect(item && "content" in item ? item.content.length : 0).toBe(8 * 1024);
  });
});
