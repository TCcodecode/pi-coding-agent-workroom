...
describe("Composer", () => {
  test("applies prefill text when the prefill key changes", () => {
    const { rerender, props } = renderComposer();
    expect(screen.getByRole("textbox", { name: /message/i })).toHaveValue("");
    rerender(
      <Composer
        {...props}
        prefill={{ key: "e5f6a7b8:1", text: "rewound prompt" }}
      />,
    );
    expect(screen.getByRole("textbox", { name: /message/i })).toHaveValue("rewound prompt");
  });
