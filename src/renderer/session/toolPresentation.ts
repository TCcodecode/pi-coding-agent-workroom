 ...
 * expandable row. Hard breaks (user/assistant text, thinking, notifications,
 * errors, in-flight tools) and non-aggregatable tools flush any pending group.
 * Thinking always keeps its own row so the trace order stays intact.
 */
...
export function thinkingSummary(content: string, opts?: { live?: boolean }): string | undefined {
  // Avoid splitting a possibly large, still-streaming block on every render.
  let line = "";
  if (opts?.live) {
    ...
    line = (newline === -1 ? content : content.slice(0, newline)).trim();
  }
  if (!line) return undefined;
