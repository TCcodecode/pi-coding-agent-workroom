...
interface TurnProps {
  ...
  lastTurnEntryId?: string;
  onUndoLastTurn?: (item: UserTimelineItem) => void;
...
export interface TimelineProps {
  ...
  onSaveInterruptedMessageEdit?: () => void | Promise<void>;
  onCancelInterruptedMessageEdit?: () => void;
  /** In-process last user entry; undo restores that turn's files and conversation. */
  lastTurnEntryId?: string;
  onUndoLastTurn?: (item: UserTimelineItem) => void;
...
export const Timeline = memo(function Timeline({
  ...
  return (
    ...
    prev.lastTurnEntryId === next.lastTurnEntryId &&
    prev.onUndoLastTurn === next.onUndoLastTurn &&
 ...
 * proportional to the viewport instead of the transcript length. Expand state
 * lives on Timeline keyed by row id so unmounting a turn does not fold it.
 */
      ...
      {entries.map((entry) => entry.kind === "toolGroup"
        ...
        : (
            <TimelineItemView
              ...
              lastTurnEntryId={lastTurnEntryId}
...
function ToolGroupView({
  ...
  const previews = group.items.slice(0, previewLimit).map((item) => ({
    id: item.id,
    text: describeTool(item).preview || toolPreview(item.input),
  }));
  const extra = group.items.length - previewLimit;
  return (
    <article className={`timeline-item tool-item tool-group completed ${dangerous ? "is-dangerous" : ""}`}>
      <button
        type="button"
        className="timeline-item-heading toggleable tool-group-toggle"
...
const TimelineItemView = memo(function TimelineItemView({
  ...
  if (item.kind === "thinking") {
    const duration = timelineDuration(item);
    const summary = thinkingSummary(item.content, { live: item.status === "streaming" });
    ...
    return (
      <article className={`timeline-item thinking-item ${item.status}`}>
        ...
        >
          ...
          <strong>{label}</strong>
          {summary && <span className="thinking-preview">{summary}</span>}
  ...
  if (item.kind === "user") {
    ...
    const showUndoLastTurn = Boolean(
      lastTurnEntryId && item.entryId === lastTurnEntryId && onUndoLastTurn,
    );
    ...
    const showRowActions = showUndoLastTurn || showInterruptedActions;
      ...
      <article className={`timeline-item message-item user${showInterruptedActions ? " is-interrupted-actionable" : ""}`}>
        ...
        {isEditingInterruptedMessage ? (
          ...
          <>
            ...
            {showRowActions && (
              <div
                className="timeline-message-actions timeline-message-actions-below"
                aria-label={showInterruptedActions ? "Interrupted message actions" : "Message actions"}
              >
                {showInterruptedActions && onCopyInterruptedMessage && (
                  <button
                    type="button"
                    className="timeline-message-action timeline-message-action-icon"
                ...
                {showUndoLastTurn && (
                  <button
                    ...
                    title="Undo this turn (conversation and files)"
                    onClick={() => onUndoLastTurn?.(userItem)}
                  >
                    <AppIcon name="undo" size="xs" />
                  </button>
                )}
                {showInterruptedActions && onEditInterruptedMessage && (
                  <button
                    type="button"
                    className="timeline-message-action timeline-message-action-icon"
                    aria-label="Edit interrupted message"
