...
export function reducePiEvent(state: AppState, event: PiEvent): AppState {
  switch (event.type) {
    ...
    case "user_message_created": {
      ...
      return {
        ...
        session: {
          ...
          status: "running",
          name: nextName,
          lastTurnEntryId: event.payload.entryId,
        },
        timeline: [
          ...state.timeline,
          {
            ...
            ...(event.payload.entryId ? { entryId: event.payload.entryId } : {}),
          },
        ],
      };
    }
    ...
    case "thinking_delta":
      return {
        ...state,
        timeline: updateTimelineItem(state.timeline, event.payload.messageId, (item) => {
          if (item.kind !== "thinking") return item;
          const content = item.content + event.payload.delta;
          return { ...item, content: content.length > 8 * 1024 ? content.slice(0, 8 * 1024) : content };
        }),
      };
