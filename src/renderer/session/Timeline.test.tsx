...
describe("Timeline", () => {
  ...
  test("offers Undo last turn only on the in-process last user turn", () => {
    const onUndoLastTurn = vi.fn();
    render(
      <Timeline
        items={[
          { id: "user-1", kind: "user", content: "Older turn", status: "completed", entryId: "aaaaaaaa" },
          { id: "user-2", kind: "user", content: "Latest turn", status: "completed", entryId: "bbbbbbbb" },
        ]}
    ...
    expect(screen.queryByRole("button", { name: "Continue from here" })).not.toBeInTheDocument();
    ...
    expect(onUndoLastTurn).toHaveBeenCalledWith(expect.objectContaining({ id: "user-2", entryId: "bbbbbbbb" }));
  });

  test("hides Undo last turn when this process has no last-turn snapshot", () => {
    render(
      <Timeline
        items={[
          { id: "user-1", kind: "user", content: "Hydrated turn", status: "completed", entryId: "aaaaaaaa" },
        ]}
  ...
  test("follows the latest thinking line while it is streaming", () => {
...
describe("Timeline tool grouping", () => {
  ...
  test("keeps thinking between tools as its own row instead of grouping across it", () => {
    const items: TimelineItem[] = [
  ...
  test("caps group previews at three with a remainder count", () => {
    render(<Timeline items={[
      bash("bash-1", "one"),
      bash("bash-2", "two"),
      bash("bash-3", "three"),
