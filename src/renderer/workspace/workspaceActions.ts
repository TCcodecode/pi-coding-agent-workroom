...
export async function rewindWorkspaceSession(): Promise<string | undefined> {
  ...
  if (useAppStore.getState().session.status === "running" || useAppStore.getState().session.status === "awaiting_approval") {
    await api.abort(tabId ? { sessionKey: tabId } : undefined);
  }
  const sessionKey = await ensureActiveTabRuntime();
  if (!sessionKey) throw new Error("Pi runtime has not been started");
  const result = await api.rewindLastTurn({ sessionKey });
